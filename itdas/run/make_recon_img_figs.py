...
from iqms import get_scr, get_smr, get_loc_err
...
def make_recon_figs(logger=null_logger):
    ...
    for expt_id in geom_params.keys():
        ...
        scr[expt_id] = get_scr(das_imgs[expt_id], ant_rad, adi_rad, tum_rad, tum_x, tum_y, normalize=True)
        smr[expt_id] = get_smr(das_imgs[expt_id], ant_rad, adi_rad, tum_rad, tum_x, tum_y, normalize=True)
        le[expt_id] = get_loc_err(das_imgs[expt_id], ant_rad, tum_x, tum_y)
...
if __name__ == "__main__":
    ...
    logger.info("Beginning...Plotting of All Reconstructed Images...")
